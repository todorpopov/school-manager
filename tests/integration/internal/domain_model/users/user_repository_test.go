...
func (suite *UserRepositorySuite) TestCreateUser() {
	testCases := []struct {
		...
		{
			name: "Successfully create user with multiple roles",
			createUser: &users.CreateUser{
				FirstName: "Multi",
				LastName:  "Role",
...
func (suite *UserRepositorySuite) TestUpdateUser() {
	testCases := []struct {
		...
		{
			name: "Successfully update user roles",
			initialUser: &users.CreateUser{
				FirstName: "Role",
				LastName:  "Changer",
				Email:     "role.changer@example.com",
				Password:  "hashedPassword111",
				Roles:     []string{"STUDENT"},
			},
			updateUser: &users.UpdateUser{
		...
		{
			name: "Successfully update user data and roles",
			initialUser: &users.CreateUser{
				FirstName: "Complete",
				LastName:  "Update",
		...
		{
			name: "Fail to update user with invalid role",
			initialUser: &users.CreateUser{
				FirstName: "Invalid",
				LastName:  "Role",
				Email:     "invalid.role.update@example.com",
				Password:  "hashedPassword333",
				Roles:     []string{"STUDENT"},
			},
			updateUser: &users.UpdateUser{

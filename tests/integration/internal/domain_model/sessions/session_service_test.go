...
type SessionServiceSuite struct {
	...
	sessionSvc sessions.ISessionRepository
	userRepo   users.IUserRepository
	rolesRepo  roles.IRoleRepository
}

func (suite *SessionServiceSuite) SetupSuite() {
	suite.TestSuite.SetupSuite()
	sessionRepo := sessions.NewSessionRepository(suite.Db, suite.Env.SessionExpiration, suite.Logger)
	suite.sessionSvc = sessions.NewSessionService(sessionRepo)
...
func (suite *SessionServiceSuite) SetupTest() {
...
func (suite *SessionServiceSuite) TearDownSuite() {
	suite.TestSuite.TearDownSuite()
}

func (suite *SessionServiceSuite) TestCreateOrRenewSession() {
	testCases := []struct {
		name           string
		useTransaction bool
		seedSession    bool
	...
	for idx, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.seedSession {
				seededSession, err = suite.sessionSvc.CreateOrRenewSession(suite.Ctx, nil, userId)
			...
			startedAt := time.Now()
			session, appErr := suite.sessionSvc.CreateOrRenewSession(suite.Ctx, tx, userId)
